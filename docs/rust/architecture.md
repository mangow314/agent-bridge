...
缺產物時 shim **大聲失敗**（exit 127＋建置提示），不靜默降級：當年的具體風險
是兩套實作靜默互換，讓套件以為在驗 Rust、其實驗到 bash（假綠）；正本退役後
理由仍成立——入口沉默地做別的事，比直接壞掉更難查。

### 10.2 source-contract 檢查改綁 Rust（含 M3 codex 的建議形）

> **狀態（bash 正本退役後）**：本節記的是 M4 當時的雙載具形狀。`SRC_KIND`
> 旋鈕與下表的「bash 源」一欄都已隨正本退役移除，實際生效的只剩「rust 源」
> 那一欄；§10.2.1 的教訓則以單向斷言的形式保留。原表留存供沿革查考。
...
### 11.4 bash 正本凍結（已結案：正本退役）

M5 當時：分組 35、36 只對 Rust 執行，`SRC_KIND=bash` 時顯式印 SKIP。bash
正本自 M4 起是 rollback 基準——它代表「回到 M4 的行為」，不該再長新功能。
parity 的形狀因此從「兩邊同數字」變成「Rust 784／bash 756＋顯式 SKIP」。

**其後 bash 正本退役**（`bin/agent-bridge.bash` 自樹上移除，git 歷史仍可
取回）：`SRC_KIND` 旋鈕與 13 組的 SKIP 分支一併塌縮，套件只剩一個受測載具。
唯一保留的是「載具身分必須實測」那道（§10.2.1 的教訓），判準從雙向比對收斂
成單向斷言——`$BRIDGE` 仍可被呼叫者覆寫，指錯照樣要紅。

### 11.5 自檢

> **狀態（bash 正本退役後）**：下表的 PASS／FAIL 是 **M5 當時的實測紀錄**，
> 原樣留存為證據。分組結構在正本退役後已塌縮（`e93ec5b`：`SRC_KIND` 旋鈕與
