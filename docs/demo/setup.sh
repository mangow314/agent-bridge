#!/usr/bin/env bash
# README demo 錄影環境（docs/demo/demo.tape 的 Hidden 階段呼叫）：
...
pkill -f 'agent-bridge/docs/demo/driver.sh' 2>/dev/null || true
rm -rf "$ROOT"
...
PROMPT_COMMAND=
PS1='\[\e[1;36m\]researcher\[\e[0m\] \[\e[2m\](codex stub)\[\e[0m\] ❯ '
RC
...
# driver.sh 編排。形狀沿用 tests/run-tests.sh 的 runtime shim。
cat > "$ROOT/shim/codex" <<EOF
#!/usr/bin/env bash
brief="\${*: -1}"
tmux select-pane -t "\$TMUX_PANE" -T 'researcher (worker)' 2>/dev/null || true
printf '\e[2m[demo stub] codex stand-in — worker brief injected (%s chars); no real API calls\e[0m\n' "\${#brief}"
...
# worker 側編排器：等任務送達後演 start → reply
setsid "$REPO/docs/demo/driver.sh" "$ROOT/data" >"$ROOT/driver.log" 2>&1 < /dev/null &
